...
function renderCatalogGrid(partsList) {
    ...
    countEl.textContent = `${currentLang === 'te' ? 'లభించిన పార్ట్స్:' : 'Showing'} ${partsList.length} ${currentLang === 'te' ? '' : 'parts'}`;
    ...
    partsList.forEach(item => {
        ...
        if (item.vehicle === "Four Wheeler") tagClass = "tag-red";
        if (item.vehicle === "Tractor") tagClass = "tag-green";
        ...
        card.innerHTML = `
            <div class="part-card-img-wrap">
                <span class="part-tag ${tagClass}">${item.vehicle}</span>
